...
namespace Core.Repositories.Mongo
{
        ...
        where TMongoEntity : class, IMongoEntity, new()
    {
        ...
        public IMongoCollection<TMongoEntity> hangfireCollection
        {
            get
            {
                return _mongoHelper.Connection.GetCollection<TMongoEntity>("hangfire.job");
            }
        }
        ...
        public List<TMongoEntity> GetHangfireList(Expression<Func<TMongoEntity, bool>> condition)
        {
            var result = hangfireCollection.Find<TMongoEntity>(condition);
            if (result.Count() == 0)
            {
                return null;
            }
